...
function createNewLine(propert:HTMLInputElement){
        
    checkCreateSpanShot(propert);
...
function checkCreateSpanShot(propert:HTMLInputElement){
...
function deleteLine(propert:HTMLInputElement){
    let split = propert.getAttribute("name")!.split(".")
    let objeto:string = split[1]
    let linha:string = split[3]

    var dependecyes = _tableDependency.filter( c=> c.key.split(".")[0] == objeto && c.key.split(".")[2] == linha);
    dependecyes.forEach(dependency => {
        let index  = _tableDependency.indexOf(dependency);
        _tableDependency.splice(index,1)
    })
    var resolveds = _resolvedDependency.filter( c=> c.split(".")[0] == objeto && c.split(".")[2] == linha);
